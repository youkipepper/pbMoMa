...
def phaseBasedMagnify(vidFname, vidFnameOut, maxFrames, windowSize, factor, fpsForBandPass, lowFreq, highFreq):
    ...
    # Convert AVI to MP4
    mp4_output_file = vidFnameOut.replace('.avi', '.mp4')
    convert_avi_to_mp4(vidFnameOut, mp4_output_file)

    print('Video saved as: %s' % vidFnameOut)
    print('MP4 video saved as: %s' % mp4_output_file)

def convert_avi_to_mp4(avi_file, mp4_file):
    cmd = "ffmpeg -i %s -c:v libx264 -preset veryfast -crf 18 -c:a aac -b:a 192k %s" % (avi_file, mp4_file)
